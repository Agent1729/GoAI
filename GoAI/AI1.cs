...
namespace GoAI
{
	public class AI1 : AI
	{
		...
		public double libertyUpTo2Mod = 1;
		public double libertyDownTo1Mod = 1;
		public double captureMod = 1;
		...
		public override TwoInts takeTurn(Board b)
		{
			...
			int x = -1;
			int y = -1;
			if (Math.Abs(b.playerScore[0] - b.playerScore[1]) > 100)
		...
		public double calculateOdds(Board b, int x, int y)
		{
			...
			foreach (Group g in b.groups)
			{
				...
				if (s2 == null)
				{
					...
					diffs.Add(new GroupDifference(g.player, -g.stones.Count, 0, -g.getLiberties(b).Count, 0, -g.getEyes(b).Count));
					continue;
				}
				Group g2 = s2.group;
				//Calculate differences between g and g2
				diffs.Add(new GroupDifference(g.player, g2.stones.Count - g.stones.Count, g2.stones.Count, g2.getLiberties(b2).Count - g.getLiberties(b).Count, g2.getLiberties(b2).Count, g2.getEyes(b2).Count - g.getEyes(b).Count));
			}
			...
			if (b2.getStone(x, y).group.stones.Count == 1)
			{
				...
				diffs.Add(new GroupDifference(s2.player, 1, 1, s2.group.getLiberties(b2).Count, s2.group.getLiberties(b2).Count, s2.group.getEyes(b2).Count));
			}
		...
		public double calculateScoreOfDiffs(List<GroupDifference> diffs, int[] territoryDiff)
		{
			...
			foreach (GroupDifference gd in diffs)
			{
				...
				if (gd.libertyDiff == 1 && gd.libertyCount == 2 && gd.player == player)
					tempScore *= libertyUpTo2Mod;
				if (gd.libertyDiff == -1 && gd.libertyCount == 1 && gd.player != player)
					tempScore *= libertyDownTo1Mod;
				if (gd.stoneCount == 0 && gd.player != player)
					tempScore = -gd.stoneDiff * gd.stoneDiff * captureMod;
		...
		public override void setAI(int _player, List<double> aivals)
		{
			player = _player;
			groupSizeMod = aivals[0];
			libertyMod = aivals[1];
			eyeMod = aivals[2];
			territoryMod = aivals[3];
			libertyUpTo2Mod = aivals[4];
			libertyDownTo1Mod = aivals[5];
			captureMod = aivals[6];
		}
		...
		public struct GroupDifference
		{
			...
			public GroupDifference(int _p, int _sd, int _s, int _ld, int _l, int _e)
			{
				player = _p;
				stoneDiff = _sd;
				stoneCount = _s;
